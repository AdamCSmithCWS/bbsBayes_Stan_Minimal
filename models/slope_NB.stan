// This is a Stan implementation of the bbsBayes slope model with a Negative Binomial error
...
// covariate effect on intercepts and slopes
  ...
  for(i in 1:ncounts){
    E[i] =  beta[strat[i]] * (year[i]-fixedyear) + strata[strat[i]] + yeareffect[strat[i],year[i]] + obs[obser[i]] + eta*firstyr[i];
  }
...
reciprocal_phi ~ std_normal(); //half normal prior on 1/phi - puts low prior probability on variance >> (E^2*2)
